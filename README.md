...
5. Let the magic happen! Every ID it comes by is logged to `LOGS/`, and every found match will be logged to `LOGS/FOUND/`.
6. If Cloudflare intercepts, the execution ends and the error log will be written to `LOGS/`.
7. If you wish to start fresh on a subsequent run, feel free to delete the log folders and/or the `comp_cache` folder.
